...
class AzurePipeline(Connection):
    ...
    def __post_init__(self):
        if not self.build_id:
            logger.error(f"Something wrong with the BuildId: {self.build_id}. Please provide an existent one.")
            sys.exit(1)
        ...
        super(AzurePipeline, self).__init__(base_url=self.organization_url, creds=credentials)
        ...
        self._build_pipeline: Build = self._build_client.get_build(build_id=self.build_id, project=self.project)
    ...
    def get_timeline(self, build_id: Optional[str] = None) -> Timeline:
        ...
        timeline: Timeline = self._build_client.get_build_timeline(build_id=build_id, project=self.project)
        if not timeline:
            logger.error("Timeline not found")
            sys.exit(1)

        if self.save_logs:
            write_json(output_path=Path(self.logs_dir, "timeline.json"), obj=self._serialize(timeline))
    ...
    def get_failed_tasks(self, timeline: Timeline) -> List[TimelineRecord]:
        ...
        if self.save_logs:
            write_json(output_path=Path(self.logs_dir, "failed_tasks.json"), obj=self._serialize(failed_tasks))

        logger.debug(f"Failed tasks have been extracted successfully -> length={len(failed_tasks)}")
        return failed_tasks
    ...
    def get_failed_tasks_logs(self, timeline: Timeline) -> Tuple[dict, dict]:
        ...
        for task in failed_tasks:
            ...
            logger.info(f"searching parent for: {task.name} with parent_id: {task.parent_id}")
        ...
        if self.save_logs:
            ...
            write_json(output_path=Path(self.logs_dir, "tasks_metadata.json"), obj=metadata)
    ...
    def compare_with_prev_build(self, prev_build, curr_build: Optional[str] = None) -> Optional[str]:
        ...
        Returns:
            Optional[BuildMessages]: feedback message that shows comparison of current build with the previous one
        """
        # Set curr_build to the current build_id if no custom one is provided
        if not curr_build:
            curr_build = self.build_id

        logger.info(f"Compare previous build: {prev_build} to current build: {curr_build}")
        ...
        logger.info(f"Out message ={message.value}")
        return message